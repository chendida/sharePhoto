package com.zq.dynamicphoto.waterutil.customview;
...
import com.zq.dynamicphoto.waterutil.customview.drawable.EditableDrawable;
import com.zq.dynamicphoto.waterutil.customview.drawable.FeatherDrawable;
...
*/
    ...
    public boolean addHighlightView(int w,int h,MyHighlightView hv,int flag) {
        /*for (int i = 0; i < mOverlayViews.size(); i++) {
            if (mOverlayViews.get(i).equals(hv))
                Log.i("PictureSlideFragment","false");
                return false;
        }*/
        if (flag != 1) {
            RectF cropRectF = hv.getCropRectF();
            float top = cropRectF.top;
            float left = cropRectF.left;
            float right = cropRectF.right;
            float bottom = cropRectF.bottom;
            int height = hv.getmImageRect().height();
            int width = hv.getmImageRect().width();
            float centerX = (left + right) / 2;
            float centerY = (top + bottom) / 2;
            float realCenterX = centerX / width * w;
            float realCenterY = centerY / height * h;
            ...
            hv.getRotationMatrix();
            hv.setMoveable(true);
            hv.setScaleable(true);
            hv.setDeleteable(true);
            hv.setup(MyApplication.getAppContext(),hv.getMatrix()
                    ,rect,cropRect,false,hv.getRotation());
        }
